...
**G10. `popen("php-config --extension-dir")` at build time.** (`php_builder.c:444`.) Build is non-hermetic: the bundle depends on whatever PHP build options were chosen by the upstream packager on the build host.
