...
| Action | Why | How |
|---|---|---|
| **M8. Install user dependencies into the embedded runtime.** 🟢 Python done. | Real apps have `requirements.txt` / `package.json`; without dep injection, hermetic bundles only work for stdlib-only programs. | Stage the vendored runtime (hardlinks via `pc_copy_or_link_tree`), `pip install --target=<stage>/site-packages -r requirements.txt` via `pc_spawn_and_wait`, embed the staged tree, remove the stage. Auto-detected from `requirements.txt` presence; opt out with `--no-install-deps`. Bundle harness asserts the user's `attrs` is importable AND the shared cache stays unpolluted. Spec: `docs/architecture/M8_USER_DEPS.md`. Node (M8-B) and PHP (M8-C) follow the same pattern; PHP gated on M1-D. |
