...
| Action | Why | How |
|---|---|---|
| **M1. Vendor or build hermetic interpreters per platform-arch.** 🟢 Python + Node end-to-end (A/B/E done). PHP plumbed but no upstream binary (D). | G1, G2. The build host's `/usr/bin/python3` (etc.) is not portable. | M1-A plumbing + `scripts/vendor-runtimes.sh` (Python via `python-build-standalone`, Node via official `nodejs.org`, SHA-256-verified). M1-B/E: new `ub_embed_runtime_tree` / `ub_extract_runtime_tree` / `ub_embed_runtime_single_as_tree`. All three POSIX builders now route through the tree format; Python and Node emit a hermetic tree when `--runtime-source` is a directory. **Hermetic Python (185 MB) and Node (186 MB) bundles both run with `PATH=/nonexistent`** — proven by `tests/bundle/run-bundle-tests.sh`. PHP builder takes `--runtime-source=<binary>` for a user-supplied static PHP (no upstream pre-built exists; `static-php-cli` self-build is the documented workaround). Spec: `docs/architecture/M1_HERMETIC_INTERPRETERS.md`. |
