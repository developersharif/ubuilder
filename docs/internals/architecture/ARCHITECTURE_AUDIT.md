...
**G1. ~~Interpreter source is the build host.~~** Closed for Python and Node. Hermetic Python (`python-build-standalone`) and Node (official `nodejs.org` linux-x64) bundles both run with `PATH=/nonexistent`, verified by the bundle harness. PHP still sources from host — no upstream pre-built static PHP exists; the builder accepts `--runtime-source=<user-built-static-php>` for users with their own `static-php-cli` artifact.

**G2. ~~Interpreter stdlib is not embedded on Unix.~~** Closed for Python (M1-B) and Node (M1-E). Hermetic Python embeds the full ~4 500-file tree (`bin/`, `lib/python3.12/`, `include/`, `share/`); hermetic Node embeds the official `nodejs.org` linux-x64 tree (~4 530 files). PHP still uses host extensions in non-hermetic mode; a user-supplied static PHP via `--runtime-source` works as long as it doesn't need separately-loaded extensions.
...
**G4. ~~No statically linked launcher.~~** Closed in S8. `cmake -DUBUILDER_STATIC=ON` produces a launcher with `ldd` reporting "not a dynamic executable". The musl toolchain (`toolchains/musl-linux-x86_64.cmake`) is the recommended path for distribution; glibc-static is a working dev fallback. CI builds and tests both. Doc: `docs/architecture/STATIC_LAUNCHER.md`.
...
**G6. Platform abstraction is missing.** Largely closed by S1+S4: `platform_compat.{c,h}` now hold 8 helpers covering spawn, env, temp-dir, executable-path, mkdir-p, path-lookup, remove-tree, and spawn-capture. `ubuilder.c` and `runtime_embedder.c` have several fewer `#ifdef PLATFORM_WINDOWS` blocks. Still open: `pc_realpath` (would replace the `resolve_binary_path` hand-rolled-symlink code in `runtime_embedder.c`), and the Windows-specific runtime-extraction branches in each `*_builder.c` (those are runtime-format work, not platform abstraction).
...
**G11. ~~Zero end-to-end test coverage.~~** Closed. `tests/bundle/run-bundle-tests.sh` covers Tier-2 (build → bundle → run → assert stdout, plus SHA-256 tamper test). `tests/bundle/run-tier3.sh` covers Tier-3: hermetic Python and Node bundles run inside `debian:12-slim` with no host runtimes installed, via Docker (stdin-piped to bypass File-Sharing restrictions) or bubblewrap (empty `/usr/bin` overlay). CI runs both tiers on every push. PHP excluded from Tier-3 until M1-D's static-PHP source lands.

**G12. ~~Stale stubs.~~** Closed: `platform_compat.{c,h}` are now populated (S1+S4), `ubuilder.c.new` was deleted, and the dead statics `validate_project_directory` / `embed_project_files` / `embed_directory_recursive` were removed (S6). The unused `is_builtin_extension` and `is_extension_enabled_in_ini` warnings in `php_builder.c` remain — those are extension-policy stubs intended for future use, kept as a placeholder until include/exclude lands.
...
| **S4. Populate `src/core/platform_compat.{c,h}` with structured spawn/fs helpers.** ✅ done. | Collapses `system("mkdir …")` shell-outs (G3) and the next batch of `#ifdef PLATFORM_WINDOWS` blocks (G6). Full surface: `pc_spawn_and_wait`, `pc_remove_tree`, `pc_env_overlay/free` (S1) + `pc_mkdir_p`, `pc_path_lookup`, `pc_spawn_capture` (S4 wave 1) + `pc_temp_root`, `pc_executable_path` (S4 wave 2). Two duplicate `/proc/self/exe`/`GetModuleFileName`/`_NSGetExecutablePath` blocks in `ubuilder.c` collapsed into a single `pc_executable_path` call site each; `get_temp_dir()` deleted in favor of `pc_temp_root()`. Linux-side `pc_executable_path` detects readlink truncation explicitly instead of silently returning a clipped path. | `src/core/platform_compat.{c,h}`, `src/core/ubuilder.c`, `src/runtimes/*` |
...
| **S6. Drop the empty `*.new` and dead-static source.** ✅ done. | Stale code & dead warnings (G12). Removed `src/core/ubuilder.c.new` (0 bytes), `validate_project_directory` (superseded by per-builder `validate_project`), and `embed_project_files` + `embed_directory_recursive` (only wrote the legacy v2 marker that S2 stopped reading). | `src/core/ubuilder.c`, deleted `src/core/ubuilder.c.new` |
| **S7. Add real end-to-end bundle tests.** | G11. See `tests/bundle/` and §5. | Done — `tests/bundle/`. |
| **S8. Statically link the `ubuilder` launcher.** ✅ done (opt-in). | G4. Closes the "launcher itself depends on build-host libc" half of the portability gap. The other half — the embedded interpreter ABI — needs M1. | `CMakeLists.txt` gained `UBUILDER_STATIC=ON`. `toolchains/musl-linux-x86_64.cmake` ships a musl-gcc / `zig cc -target x86_64-linux-musl` cross-compile. Bundle harness accepts `UBUILDER_BIN=…` env override; CI builds + tests the static launcher. `ldd` confirms zero dynamic deps. Doc: `docs/architecture/STATIC_LAUNCHER.md`. |
| **S10. Auto-vendor on first build (DX).** ✅ done. | Today the user has to run `scripts/vendor-runtimes.sh` once before they get a hermetic bundle. With auto-vendor, the first `ubuilder` invocation detects the empty cache and spawns the script transparently. | `ub_find_vendor_script` probes `$UBUILDER_VENDOR_SCRIPT` then well-known relative paths from the executable. `ub_auto_vendor("python"|"node")` spawns `bash <script> <runtime>` via `pc_spawn_and_wait`. On any failure (missing script, no network, non-zero exit) the build falls back to the host runtime with a clear note. `--no-auto-vendor` flag for CI to fail loudly instead. `fflush(stdout)` before spawning so our header appears before the script's curl output. |
...
| Action | Why | How |
|---|---|---|
| **M8. Install user dependencies into the embedded runtime.** 🟢 Python (A) + Node (B) done; PHP (C) gated on M1-D. | Real apps have `requirements.txt` / `package.json`; without dep injection, hermetic bundles only work for stdlib-only programs. | Python: stage the vendored runtime (hardlinks via `pc_copy_or_link_tree`), `pip install --target` via `pc_spawn_and_wait`, embed, remove stage. Node: stage the **project** (so `node_modules/` lands relative to the script), run `bin/node lib/node_modules/npm/bin/npm-cli.js install --omit=dev` via `pc_spawn_and_wait` with cwd=staged-project, embed, remove stage. Both auto-detected from manifest presence; opt out with `--no-install-deps`. Harness asserts: Python's `attrs` imports + cache stays clean; Node's `picocolors` imports + user's project dir stays clean. Spec: `docs/architecture/M8_USER_DEPS.md`. |
| **M1. Vendor or build hermetic interpreters per platform-arch.** 🟢 Python + Node end-to-end with Tier-3 (A/B/C/E done). PHP M1-D: Linux uses host-bits synthesis; macOS auto-detects statically linked host PHP (Herd, `static-php-cli`) and ships the binary as-is — same hermetic shape, no enumeration step. | G1, G2, G11. | M1-A: `scripts/vendor-runtimes.sh` (Python + Node, SHA-256-verified). M1-B/E: tree-format embed/extract; hermetic bundles for both runtimes. **M1-C: `tests/bundle/run-tier3.sh` runs hermetic bundles inside `debian:12-slim` (Docker, stdin-piped) or via bubblewrap (empty `/usr/bin`)** — strongest hermeticity proof short of running on an actual fresh machine. CI wires Docker mode. PHP builder accepts `--runtime-source=<static-php-binary>` (M1-D plumbing); the macOS path additionally `stat()`s the probed `extension_dir` and treats a non-existent dir as "everything's compiled in," covering `static-php-cli` output without a vendored tarball. Spec: `docs/architecture/M1_HERMETIC_INTERPRETERS.md`. |
