...
# UBuilder Examples Build and Test Script for Linux
    ...
    case $runtime in
        ...
        "php")
            if command_exists php; then
                ...
                log_warning "PHP runtime not available"
                return 1
            fi
            ;;
        "node")
            if command_exists node; then
                ...
                log_warning "Node.js runtime not available"
                return 1
            fi
            ;;
        *)
...
# Function to build UBuilder core
build_ubuilder() {
    log_header "Building UBuilder Core (Linux)"
...
# Function to build example project
build_example() {
    local example_dir=$1
...
# Main execution
main() {
    log_header "UBuilder Examples Build and Test (Linux)"
    ...
    if [ ${#built_examples[@]} -gt 0 ]; then
        ...
        # Final summary
        log_header "Build and Test Summary (Linux)"
        ...
        if [ ${#run_successful[@]} -eq ${#built_examples[@]} ] && [ ${#built_examples[@]} -gt 0 ]; then
            echo -e "\n${GREEN}🎉 ALL BUILDS AND TESTS SUCCESSFUL ON LINUX! 🎉${NC}"
