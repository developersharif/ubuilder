...
# Test all built example executables on Linux without host runtimes available
...
echo "=============================================="
echo "UBuilder Linux Portability Test (No Runtimes)"
echo "=============================================="
...
test_executable() {
    ...
    # Check if timeout command is available, provide fallback if not
    if command -v timeout >/dev/null 2>&1; then
        # Use system timeout command
        set +e  # Disable exit on error for timeout command
        timeout "${timeout_seconds}s" "$exe_path" > "${exe_name}_output.log" 2>&1
        exit_code=$?
        set -e  # Re-enable exit on error
    else
...
# Main execution
main() {
    echo "Starting Linux portability test..."
    ...
    if run_all_tests; then
        echo ""
        echo "✅ Linux portability test completed successfully!"
        exit 0
    else
        echo ""
        echo "❌ Linux portability test failed!"
        exit 1
    fi
}
